      ...
      <template #body>
        <UContainer class="max-w-7xl space-y-6">
          <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <UPageCard title="Invoice value" :description="`${data.invoiceCount} invoices issued`">
              <p v-for="total in data.invoiceTotals" :key="total.currency" class="text-sm text-muted">
                {{ money(total.amount, total.currency) }} across {{ total.count }} invoices
              </p>
              <p v-if="!data.invoiceTotals.length" class="text-sm text-muted">No invoices yet</p>
            </UPageCard>
            <UPageCard title="Approved transfers" description="Payments approved by the Owner">
              <p v-for="total in data.approvedPaymentTotals" :key="total.currency" class="text-sm text-muted">
                {{ money(total.amount, total.currency) }} across {{ total.count }} Payments
              </p>
              <p v-if="!data.approvedPaymentTotals.length" class="text-sm text-muted">No approved Payments yet</p>
            </UPageCard>
            <UPageCard title="Refunds pending" description="Approved, awaiting manual transfer">
              <p v-for="total in totalsFor('PENDING')" :key="total.currency" class="text-sm text-muted">
                {{ money(total.amount, total.currency) }} across {{ total.count }} Refunds
              </p>
              <p v-if="!totalsFor('PENDING').length" class="text-sm text-muted">No pending Refunds</p>
            </UPageCard>
            <UPageCard title="Refunds completed" description="Manual transfers recorded">
              <p v-for="total in totalsFor('COMPLETED')" :key="total.currency" class="text-sm text-muted">
                {{ money(total.amount, total.currency) }} across {{ total.count }} Refunds
              </p>
              <p v-if="!totalsFor('COMPLETED').length" class="text-sm text-muted">No completed Refunds</p>
            </UPageCard>
          </div>

          <UPageCard title="Payment status">
            <UTable :data="statuses.map(status => ({ status, count: data.payments[status] }))" :columns="[
              { accessorKey: 'status', header: 'Status' },
              { accessorKey: 'count', header: 'Payments' },
            ]" />
          </UPageCard>

          <UPageCard title="Invoices" description="Issued amounts, grouped by currency. Invoice value is not cash received.">
          ...
          <UPageCard title="Refund records" description="Each Refund stays pending until its transfer date and reference are recorded.">
            <UTable :data="data.refunds" :columns="[
              ...
              <template #amount-cell="{ row }">{{ money(row.original.amount, row.original.currency) }}</template>
              <template #transferredAt-cell="{ row }">{{ row.original.transferredAt ? new Date(row.original.transferredAt).toLocaleDateString() : 'Pending' }}</template>
            </UTable>
          </UPageCard>

          <UPageCard title="Audit history" description="Recent operational and financial changes">
            <UTable :data="data.auditEvents" :columns="[
              ...
              <template #target-cell="{ row }">{{ row.original.entityType }} {{ row.original.entityId }}</template>
              <template #actor-cell="{ row }">{{ row.original.actorName ?? row.original.actorId ?? 'System' }}</template>
