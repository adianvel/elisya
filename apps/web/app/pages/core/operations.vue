    ...
    <UDashboardPanel>
      ...
      <template #body>
        <UContainer class="max-w-7xl space-y-6">
          <UPageCard
            title="Trips and vehicle assignments"
            :description="`${data.trips.length} Trips, capacity shown by Trip`"
          >
            <UTable
              :data="data.trips"
              :columns="[
            ...
            >
              <template #departureAt-cell="{ row }">
                {{ new Date(row.original.departureAt).toLocaleString() }}
              </template>
          ...
          <div class="grid gap-6 lg:grid-cols-2">
            <UPageCard
              ...
              :description="`${data.pendingPaymentReviews.length} waiting for a decision`"
            >
              <UTable
                :data="data.pendingPaymentReviews"
                :columns="[
              ...
              >
                <template #trip-cell="{ row }">
                  {{ row.original.origin }} to {{ row.original.destination }}
                </template>
            ...
            <UPageCard
              ...
              :description="`${data.cancellationCases.length} waiting for a decision`"
            >
              <UTable
                :data="data.cancellationCases"
                :columns="[
          ...
          <UPageCard
            title="Vehicles"
            description="Track each unit and assign it to an upcoming Trip."
          >
            <form
              class="mb-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto]"
