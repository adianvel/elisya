  ...
  <main>
    <section class="mx-auto grid max-w-6xl gap-10 px-5 pb-14 pt-10 sm:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:pt-20">
      <div>
        ...
        <h1 class="max-w-2xl text-5xl font-semibold leading-[0.98] tracking-[-0.06em] text-[#103f38] sm:text-7xl">Your next route starts here.</h1>
        <p class="mt-6 max-w-xl text-lg leading-8 text-[#52706a]">Browse available routes, then book and send your transfer receipt through WhatsApp. Your WhatsApp number keeps your booking connected to you.</p>
      </div>
      <div class="rounded-[2rem] bg-[#103f38] p-6 text-[#f4f7f5] shadow-[0_20px_60px_rgba(16,63,56,0.18)] sm:p-8">
        <p class="text-sm font-medium text-[#b9d0c8]">Booking happens in WhatsApp</p>
        <h2 class="mt-4 text-2xl font-semibold">Pick a route below</h2>
        <p class="mt-3 text-sm leading-6 text-[#d7e5df]">Message the Palawa travel team with your route and departure. They will help you reserve seats and send your payment proof in the same conversation.</p>
      </div>
    </section>
    ...
    <section class="mx-auto max-w-6xl px-5 pb-20 sm:px-8">
      <div class="rounded-[1.75rem] border border-[#d6e3de] bg-white p-5 shadow-sm sm:p-8">
        ...
        <div v-else class="mt-6 grid gap-3 sm:grid-cols-2">
          <article v-for="trip in trips" :key="trip.id" class="rounded-2xl border border-[#dce9e4] p-4">
